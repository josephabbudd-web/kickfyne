...
// The {{ $panelName }}TabItem uses the local {{ $panelName }} panel for content.
...
// Open{{ $panelName }}TabItem constructs a {{ $panelName }}TabItem.
// The {{ $panelName }}TabItem uses the {{ $panelName }} screen for content.
func Open{{ $panelName }}TabItem(screen *_misc_.Miscellaneous, preset *_presetting_.{{ $panelName }}ScreenPreset) (err error) {
	...
	if appTabsTabItemContentConsumer, {{ call $DOT.Funcs.DeCap $panelName }}ScreenID, err = api.NewAppTabsTabItemContentConsumer(
		screen.CTX,
		screen.CTXCancel,
		screen.APP,
		screen.Window,
	...
	screen.Layout.AddRemoteScreenIDTabItemConsumer({{ call $DOT.Funcs.DeCap $panelName }}ScreenID, tabItem, appTabsTabItemContentConsumer)
	return
}
{{- end }}
