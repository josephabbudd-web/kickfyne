...
// RemoveItems removes items from a DocTabs screen package.
func RemoveItems(
	packageName string,
	...
	defer func() {
		if err != nil {
			err = fmt.Errorf("apptabs.RemoveItems: %w", err)
		}
	}()
	...
	files := files(packageName, removeLocalItemNames, folderPaths)
