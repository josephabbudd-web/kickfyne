...
import (
	...
	"github.com/josephabbudd-web/kickfyne/gomod"
)
...
func WriteGitKeepFile(folderPath string) (err error) {
	err = WriteFile(
		filepath.Join(folderPath, KeepFileName),
		[]byte("Git, keep this folder."),
	)
	return
}
...
// If parentFolderPath does not exists: len(names) == 0, err == nil.
func FolderNames(parentFolderPath string) (names []string, err error) {
	...
	if dirEntrys, err = os.ReadDir(parentFolderPath); err != nil {
		if os.IsNotExist(err) {
			// It's ok if the parent folder path does not exist.
			// names = make([]string, 0)
			err = nil
		}
		return
...
// FrameworkIsBuilt returns if the framework was built in this folder.
// It does so by checking for 3 folders.
func FrameworkIsBuilt(appPath string) (isBuilt bool, err error) {
